function emojiBear(x, y, s, sentence) {
  // Ears (Large)
  fill("#855C52");
  stroke("#855C52");
  ...
  stroke("#7D695C");
  fill("#7D695C");
  rect(x + 60 * s, y - 255 * s, 235 * s, 50 * s, 40);
  triangle(
    x + 170 * s,
    y - 205 * s,
    x + 145 * s,
  ...
  textSize(20 * s);
  textStyle();
  text(sentence, x + 87 * s, y - 223 * s);
}

x = canvas.width / 2;
y = canvas.height / 2;
let w = canvas.width;
let sentence = "I need coffee ASAP!";

function draw() {
  background(228, 227, 218);
  emojiBear(x, y + 50, 0.9, sentence);
}

function mouseClicked() {
  const randomNumber = Math.floor(random() * 3);
  if (randomNumber === 0) {
    sentence = "Where is my coffee?";
  } else if (randomNumber === 1) {
    sentence = "Did you see coffee?";
  } else if (randomNumber === 2) {
    sentence = "I really love coffee!";
  }
  emojiBear(x, y + 50, 0.9, sentence);
}
